...
# local versions of build_template
...
def intersperse(lst, thing):
    result = [thing] * (len(lst) * 2 - 1)
    result[0::2] = lst
    return result
...
def registration(
    ...
    **kwargs
    ...
    if isinstance(initial_fixed_transform, str):
...
#    fixed = ants.iMath( fixed.clone("float"), "Normalize" )
    ...
    if initx is None:
        ...
        # need to interleave -r options, but not the first one
        initx = intersperse(initx, "-r")

    if initial_fixed_transform is not None:
        initfx = ["-q"]
        initfx = initfx + intersperse(initial_fixed_transform, '-q')
        initx = initx + initfx
    # ------------------------------------------------------------
...
def build_template(
    ...
    **kwargs
    ...
    def avgUpdate(im):
        # to mimic the bash behaviour. Finally confirmed that
        # the sharpening is applied to the average, never the
        # input.
        imnorm = im/im.mean()
        return imnorm
    ...
    for i in range(iterations):
        ...
        for k in range(len(image_list)):
            ...
            if k == 0:
                ...
                # this is where normalization and sharpening would go if
                # we want to be equivalent to the bash script
        ...
        if L == 2:
            print(wavg.abs().mean(), flush = True)
